<template>
  <div class="app-container">
    ...
    > 
      ...
      <el-table-column label="优惠券面值" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.value.price }}</span>
        </template>
      </el-table-column>
      <el-table-column label="适用课程" width="500px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="使用期限" width="200px" align="center">
        <template slot-scope="{row}">
          ...
          <div>{{ row.updated_time }}</div>
        </template>
      </el-table-column>
      <el-table-column label="发行量" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.c_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已领取" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.received_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已使用" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.used_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="秒杀状态" width="100px" class-name="status-col" align="center">
        <template slot-scope="{row}"     >
          <el-tag  type="row.status === 1 ? 'success' : 'danger' " >
            {{ row.status === 1 ? '领取中' : '已下架' }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" fullscreen:true>
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="拼团价格">
           <el-input-number v-model="temp.value.price"  :min="1"  label="描述文字"></el-input-number>
        </el-form-item>
        <el-form-item label="拼团人数">
           <el-input-number v-model="temp.c_num"  :min="1"  label="描述文字"></el-input-number>
        </el-form-item>
        <el-form-item label="秒杀活动开启时间-结束时间" >
...
import { fetchCoupon,  fetchPv, createCoupon, updateCoupon } from '@/api/marketing'
