<template>
  <div class="app-container">
    ...
    > 
      ...
      <el-table-column label="成团人数" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.p_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="拼团时限" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.expire }}</span>
        </template>
      </el-table-column>
      <el-table-column label="拼团状态" width="150px" class-name="status-col" align="center">
        <template slot-scope="{row}"     >
          <el-tag  type="row.status === 1 ? 'success' : 'danger' " >
            {{ row.status === 1 ? '拼团中' : '已下架' }}
          </el-tag>
        </template>
      </el-table-column>
          ...
          <el-button type="primary" @click="dialogTableVisible = true">打开嵌套表格的 Dialog</el-button>
            <el-dialog title="收货地址" :visible.sync="dialogTableVisible">
                ...
                <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left"  style="width:900px; height:600px margin-left:50px;">
                        <el-form-item>
                                <el-tabs tab-position="left" style="height: 600px;" v-model="activeName" @tab-click="handleClick">
...
import { fetchGroup,  fetchPv, createGroup, updateGroup } from '@/api/marketing'
